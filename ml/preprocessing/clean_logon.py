"""
Clean logon.csv
Input : data/raw/logon.csv
Output: data/cleaned/logon_clean.csv

Columns expected: id, date, user, pc, activity (Logon/Logoff)
...
def clean_logon(input_path: str, output_path: str) -> None:
    logger.info(f"Starting to clean logon data from {input_path}")
    try:
        ...
        logger.info(f"Loaded {len(df)} raw logon records")
        ...
        # Save output
        logger.info("Saving cleaned logon records...")
        df.to_csv(output_path, index=False)
        logger.info(f"Successfully saved {len(df)} cleaned logon records to {output_path}")

    except Exception as e:
        logger.error(f"Error cleaning logon data: {e}", exc_info=True)
        raise

if __name__ == "__main__":
    clean_logon("data/raw/logon.csv", "data/cleaned/logon_clean.csv")
